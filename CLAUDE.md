...
| **Screen goes BLANK / camera freezes walking ACROSS a zone boundary** (player walks off-screen) | the zone-transition blend interpolated the camera EYE from a frozen snapshot (`blend.pos=CAMRIG.pos.clone()`) while the look-target tracked the LIVE old-zone rig — so the eye stuck in place while the player kept walking, and on boundary churn the blend re-snapshotted the frozen eye → permanent freeze → off-screen → blank (Christopher: walking plaza→pet-shop after #30 moved the shop onto that road). FIX: blend BOTH eye + target from the two zones' LIVE rigs (`want.pos=A.pos.lerp(B.pos,u)`, `A=rigFor(fromZone)`) so the camera keeps FOLLOWING through the transition. **⚠️ This was REAL-TIME-LOOP-ONLY** — `cameraQA.path` (fixed-dt stepper) did NOT reproduce it, so it needed LIVE browser testing (drive the deployed/served game in Chrome via the MCP). Guard: `checks/camera-walk.mjs` (continuous-walk visibility across zone crossings, both ways) catches the static class; for camera-FOLLOW feel, drive it live. |
